...
mongo_cookbook = mongo_connection.db('eat_it_app_development')
mongo_collection = mongo_cookbook.collection('recipes')
...
class Recipebook
	...
	def ingredient_split(array)
		newarray = array.map{|x| x.split}.flatten.uniq.select{|x| x[0..1]=='[['}.map{|x| x.gsub(/[^a-zA-Z]/,'').downcase}
		newarray
	end
	...
	def build_relations
		1.upto(@count-1) do |num1|
			(num1+1).upto(@count) do |num2|
				...
				@cookbook[num1][:match][num2.to_s] = intersection_percent_1 if intersection_percent_1 > 0.1
				@cookbook[num2][:match][num1.to_s] = intersection_percent_2 if intersection_percent_2 > 0.1
			end
		end	
